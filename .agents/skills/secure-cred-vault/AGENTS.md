...
## Categories — organize by project (preferred)

Use the **project/product name** as the category and prefix the provider on the
key: `cursor/cloudflare_api_token`, `cursor/cloudflare_account_id`,
`cursor/github_token`. This machine hosts many apps that share `lorapok.tech`, so
project categories prevent cross-wiring. `cursor` = the Cursor Curse Monitor app.
Shared, non-project secrets may still use provider categories (`openai`, `stripe`).

**Always store:** any new key (from the user, you, or any AI) goes into the vault
under the right project category immediately; then reference it by name.

## Reset / recovery ("PIN") system

- `cred reset-pass` — reset a forgotten passphrase via the recovery key (no data loss).
- `cred recovery-show` / `cred recovery-init` — show / regenerate the recovery key.
- `cred reset --wipe` — destructive wipe + fresh init.
- Recovery key lives at `/mnt/NewVolume/Personal_Projects/cred/.recovery.key` (0600),
  with a mirror `credentials.recovery.gpg`.
- Automation env: `CRED_PASSPHRASE`, `CRED_RECOVERY_KEY`, `CRED_VALUE` (set only).

## Browser automation

Always use the user's **real default Chrome with the `Maizied` profile** (already
logged in) — never a throwaway automation browser. See
`/mnt/NewVolume/Personal_Projects/cred/BROWSER_PREFERENCE.md` and the Cursor rule
`~/cursor-usage-monitor/.cursor/rules/browser-profile.mdc`.
